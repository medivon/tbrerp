...
export function OrderLineCard({
  ...
  return (
    <article className="grid gap-3 border-b border-border p-4 last:border-b-0 md:grid-cols-[76px_minmax(0,1fr)_auto] md:items-start">
      ...
      <div className="min-w-0 space-y-2">
        ...
        {line.customDetail ? (
          <p className="rounded-md bg-subtle px-3 py-2 text-sm leading-6 text-foreground">
            {line.customDetail}
          </p>
        ) : null}
        ...
        {line.job ? (
          <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{line.job.status}</span>
            <span>{line.job.currentDepartment}</span>
            {line.deliveryDate ? (
              <span>กำหนดส่ง {line.deliveryDate}</span>
            ) : null}
          </div>
        ) : null}
