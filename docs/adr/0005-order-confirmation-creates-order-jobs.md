...
`JOB-O` creation is not gated by `Payment Record`. Payment Term and Payment Record are visible commercial/financial information, but missing payment evidence does not require a special override to create `JOB-O`.

## Considered Options

- Confirm the Order first, then send custom lines to a separate `รอสร้าง Job` queue.
- Require Payment Record before creating customer `JOB-O`.
- Create `JOB-O` immediately from complete custom lines at Order confirmation.

## Consequences

The Order Create/Edit screen must collect enough `รายละเอียดงานสั่งทำ` for each custom line before confirmation. Payment follow-up remains separate from Job creation. After confirmation, production-affecting changes must go through Job Detail / Job Revision rather than being silently edited from Order Detail.
