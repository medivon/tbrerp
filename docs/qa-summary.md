...
A: เมื่อ Admin/Sales ที่อยู่ใน scope งานของตัวเอง หรือ Finance/Manager/Owner ตามสิทธิ์ ปิดรายการพร้อมหลักฐานรับเงินที่บังคับใช้ หรือหมายเหตุที่อธิบายได้พอสำหรับ audit trail. ยังไม่ใช่ full accounting approval.

**Q: What is the starting finance scope?**
A: Operational payment follow-up only: COD, payment follow-up, refund expense, Expense Entry, and Payment Voucher. It is not full accounting or tax-ready finance.

**Q: What is the main finance work object?**
A: Financial Follow-up.

**Q: Can COD follow-up close before Shipment close?**
A: No. COD follow-up is created automatically when the final Order-closing Shipment carries COD, but it cannot be closed before the related Shipment is closed.

**Q: What if actual COD is lower than expected?**
A: Record actual amount and reason, such as carrier fee deduction. This does not change the Order total.

**Q: What if actual COD is higher than expected?**
A: This should not normally happen. If it happens, treat it as an abnormal note/correction case, not automatic extra income or heavy approval flow.

**Q: Can Payment Record be corrected?**
A: Yes, but not silently. Keep old value, new value, reason, editor, edit time, and make the correction visible to finance follow-up.

**Q: What does Payment Record require?**
A: Payment date/time, amount, payment method, evidence slip/photo, related object, and recorder. Evidence is required in the starting workflow.

**Q: Is deposit a separate module?**
A: No. Deposit is a partial Payment Record.

**Q: What is the default payment method?**
A: Controlled list, defaulting to `โอนเงิน`.
...
### Identity & Access Consolidation IA-001 to IA-150

**Q: Is Identity & Access discovery complete enough for first implementation docs?**
A: Yes. IA-001 to IA-150 are covered in six checkpoint batches and consolidated into source-of-truth docs.

**Q: What is a User?**
A: คนในระบบ identity เดียวกัน ไม่ว่าจะเป็นพนักงานภายในหรือ Outsource. Employee ไม่ใช่ระบบ login แยก แต่เป็น User Type `พนักงานภายใน`.

**Q: Who can login?**
A: Active Users can login through one login page. Base roles land on Personal Dashboard; operational menus/actions depend on role/permission.

**Q: What are the starting roles?**
A: `Super Admin / Owner`, `Manager`, `Admin / Sales`, `Finance`, `Product / Stock`, `Woodwork`, `Coloring`, `Delivery Team`, `พนักงานไทยโบราณ`, `Outsource`, and `Rak Samuk Worker`.

**Q: Who manages roles and permissions?**
A: Only `Super Admin / Owner`. Role/Permission changes take effect immediately and write Audit Log, but do not require a reason in the first scope.

**Q: What happens when a User is deactivated?**
A: Owner/Manager can deactivate with reason and Management Log. Login stops and the User cannot be selected for new work, but history/documents/income/logs remain visible by permission with `ปิดใช้งาน` badge.

**Q: Do department workers see full Job detail?**
A: Woodwork, Coloring, and assigned Rak Samuk Worker see Full Production Job Detail needed for quality, but not customer identity, Order ID, sales price, payment, cost, payout, Management Log, or Audit Log.

**Q: Can Delivery Team see COD?**
A: Yes, but only for Shipments they are responsible for. Delivery Team cannot change COD or close COD/payment follow-up.

**Q: How are completed Orders corrected?**
A: Completed Orders are not edited in normal workflow. Use notes, Service Case, finance note, correction record, or Owner/Manager-only `รอบจัดส่งพิเศษ` that does not affect stock or Order completion.

**Q: What can never be physically deleted?**
A: Critical history and evidence such as Audit Log, Action Log, payment/delivery evidence, User, Order, and Job. Use deactivate, hide, or correction instead.

## 5. Interaction & Modal Behavior

**Q: Is the Interaction & Modal Behavior discovery complete enough for source-of-truth docs?**
A: Yes. IM-001 to IM-135, including the PV addendum, are consolidated. Use `docs/ux-ui/04-interaction-modal-behavior.md` as the active behavior source.

**Q: What is the reusable permission behavior?**
A: Missing-permission actions are hidden. State-blocked actions remain visible but disabled with a clear reason. Direct URL/no-access cases show a no-access page with only return-to-own-home.
...
A: No. COD is allowed only on the final Shipment round that completes delivery for the Order. If ready-stock goods can ship while custom work remains unfinished, COD is disabled with a reason.

**Q: What is the delivery evidence boundary?**
A: Delivery Team can mark `ส่งออกแล้ว` without Tracking or photo. Delivery can add photo/note from `ส่งออกแล้ววันนี้` until Admin closes. Admin closes only after Tracking or at least one delivery evidence photo exists.

**Q: What is the PV/payout boundary?**
A: `รายการรอจ่าย` / `ตัดรอบจ่าย` is the editable payout workspace. Payable items are item-first and grouped by payee. PV is the finalized one-payee payment document and not the editable long price-review list.

**Q: When is a Rak Samuk payable item created?**
A: When work is received back / accepted into coloring, not when assigned. If price is missing, it appears as `ยังไม่มีราคา` and cannot be included in PV.

**Q: Is missing or wrong production detail a system workflow?**
A: No. Workers ask outside the system. There is no special note/action/log requirement and this should not remain an open question.

**Q: What are the important stock/product interaction decisions?**
A: Product/SKU deactivation blocks with reasons and record links when unsafe, and allowed deactivation requires reason + Management Log. Reopen needs confirmation + Management Log but no reason. Stock Count close uses a Review page. Product Stock Receipt and Material Adjustment use review/confirmation before stock-changing save. Wrong stock/material movements are corrected by new referenced adjustments, not by editing old movements.
...
**Q: What is the Admin Dashboard's primary job?**
A: It is a shared operational queue launcher, not a full task table or analytics dashboard.

**Q: Are Dashboard cards configurable in the starting workflow?**
A: No. Use the fixed six confirmed cards first.

**Q: What does each Dashboard card count?**
A: `ออเดอร์ที่ต้องติดตาม` counts real Orders needing operational follow-up. `งานกำลังผลิต` counts active Jobs. `รอสร้างรอบจัดส่ง` counts Orders/Service items ready for Shipment creation. `ยืนยันการจัดส่ง` counts sent-out Shipment rounds waiting admin close. `งานผลิตต้องติดตาม` counts Jobs needing admin communication/decision/follow-up. `ติดตาม COD / Payment` counts unresolved Financial Follow-up items.

**Q: How often do Dashboard counts update?**
A: On page load and after important actions. Realtime updates are later.

**Q: Should Admin Dashboard show money amounts?**
A: No. It may show operational finance counts, but not baht amounts.
...
## 7.2 Order Detail and Order Line Editing
...
A: No. Refund/credit จากบริบท Order เป็น note/financial follow-up/customer record ใน first scope. Order ที่จบแล้วไม่ถูกเปิดใหม่เพราะมี refund/credit. Refund จาก Service Case แยกไปเป็น Expense Entry.
...
## 8. Payment and Finance Follow-up
...
A: This should not normally happen. If it happens, treat it as abnormal COD and require a note/correction; do not auto-create extra income.
...
A: For a formal Job Revision, choose `รับทราบ` or `ไม่เข้าใจให้ติดต่อหา`. Ordinary missing/wrong production detail is outside system workflow.
...
A: Yes, limited to own assigned work. Rak Samuk prices must not appear on Order, Production, or Job workflow pages.
...
A: Rak Samuk Worker can use `ขอเสนอราคา` for assigned work until the related payable item is included in a finalized PV.
...
A: Owner/Manager. Finance pays or creates PV from the approved price.

**Q: If standard rate exists, can worker request price change in system?**
A: Yes. They use `ขอเสนอราคา` until the related payable item is included in a finalized PV.

**Q: Where is Rak Samuk standard rate stored?**
A: Product Model (SKU ใหญ่).

**Q: What if approved paid rate differs from standard rate?**
A: If linked to SKU/Product Model, the Owner/Manager approver must choose whether to update the standard rate. It is not automatic.

**Q: Does standard rate change affect old work?**
A: No. Only future work.

**Q: What happens to custom job Rak Samuk cost?**
A: It stays in Rak Samuk / Finance / PV / Owner/Manager approval/reporting context. It is not shown on Order, Production, or Job workflow pages.
...
A: The user must choose the Rak Samuk Worker before confirmation; if the worker is not known, do not start the `JOB-P` at Rak Samuk.
...
## 16. Product, SKU, and Product Settings
...
A: No. If work is not ready to ship, it should not enter the ready-to-ship queue.

**Q: When is bulk shipment allowed?**
A: Ready-stock-only Orders with no custom-work line; one Order one Shipment. Service Case can be included if details complete.

**Q: Does COD block bulk?**
A: COD can appear only when the bulk-created Shipment is the final Order-closing round.
...
A: Change items, address, carrier, Tracking, COD, close Shipment, or close COD/payment follow-up. Delivery Team can see COD amount only for Shipments they are responsible for.
...
**Q: Does Service Case require Customer or Order?**
A: Customer is required. Old Order reference is optional and used only for context.

**Q: How is Service Case refund handled?**
A: Record it as Expense Entry only, with Service Case as context. It does not change the original Order or Customer Sales Summary.

**Q: Can Service Case have a paid customer charge?**
A: Yes, use separate Financial Follow-up or payment note. Do not edit the original Order.
...
**Q: Does closing Service Case require note?**
A: No.

**Q: Can a closed Service Case be reopened?**
A: No in the starting workflow. Create a new Service Case if more after-sales work is needed.

**Q: How should Service Case report work first?**
A: Count by type/status only.
...
## 22.1 Product Stock In / Count / Adjustment
...
## 22.2 Light Material Stock / Material Boundary
...
**Q: Does receiving a Material Purchase Order create Draft Expense Entry or Payment Voucher automatically?**
A: No. Finance later records actual payment/expense deliberately from the Payment Audit Follow-up.

**Q: Should material boundary grilling continue now?**
A: No. The Product / SKU / Stock / Material boundary is coherent enough for the starting workflow; deeper supplier payment, material reporting, or future multi-supplier behavior stays future scope until explicitly reopened.
...
**Q: Can one PV include multiple Rak Samuk works?**
A: Yes, one PV can include multiple ready payable lines for one payee/worker, such as Rak Samuk work and custom income. Do not mix multiple payees into one PV.
...
### Dashboard and Report Decision Gates

**Q: Should Admin Dashboard show Owner?**
A: No. Owner remains traceability data, but not a main dashboard signal.

**Q: Should Admin Dashboard show Current Handler or next department?**
A: No. The main dashboard stays at queue/card level. Current Handler belongs in detail screens or working queues.

**Q: What should Manager Dashboard emphasize?**
A: Unfinished work, urgency, blockers, and age.

**Q: What is the first management report?**
A: Unfinished work by department/status.

**Q: What is the first sales report?**
A: Sales by period from real Orders.

**Q: Are cancelled Orders included in sales reports by default?**
A: No. Exclude cancelled Orders by default, with a separate filter/view for cancelled Orders.

**Q: Which date does the sales report use?**
...
**Q: Should reports export in the starting workflow?**
A: No general report export in the starting workflow.

**Q: What layout should reports use first?**
A: Summary cards plus tables.

**Q: Should Dashboard show stock-negative warning?**
A: Yes, as a warning summary where relevant.

**Q: Should low-stock be on the main Dashboard?**
A: No. Keep it in stock/material screens first, or a later stock module summary.

**Q: Should Service Case count appear on the main Admin Dashboard first?**
A: No.

**Q: Where should overdue delivery appear?**
A: In `งานที่ต้องรีบดู` / critical preview and relevant queues.

**Q: Where should waiting-materials work appear?**
A: Through production/material signals or critical preview, not as a separate notification module.

**Q: Should the starting dashboard have personal task mode?**
A: No. Shared queues first.

**Q: Where should report navigation live?**
A: In relevant modules and selected dashboard shortcuts, not as a heavy separate analytics area first.

**Q: Is individual performance reporting included now?**
A: No. Preserve data for a later User Dashboard, but do not build the scorecard now.

**Q: What is the Dashboard/Report design principle?**
A: Operational clarity before analytics.

### Notification and Alert Decision Gates

See ADR `0015-operational-alerts-are-queue-status-events.md`.

**Q: What is the starting notification model?**
A: In-app alerts on the actual working screens, using queues, badges/counts, chips, inline warnings, and critical previews.

**Q: Should notifications be separated by module/event?**
A: Yes. Event names and module boundaries should be clear so future API/webhook/external hooks can be added.

**Q: How are urgent Jobs shown?**
A: Clear urgent label on the queue/work card.

**Q: How are near-delivery and old work shown?**
A: Near-delivery uses sorting, chip, and critical preview. Old work uses aging chips and manager overview.

**Q: How are waiting materials shown?**
A: Through waiting-material lists, Material Stock, or production blocker signals.

**Q: Does material receipt create a separate notification?**
A: No. It releases linked Jobs where applicable and writes the log.

**Q: How are Job Revision, Rak Samuk missing price, COD, and Payment Audit alerts shown?**
A: Through their owning queues/cards: revision acknowledgement on work card/queue, Rak Samuk missing-price approval queue, COD/Payment queue, and finance follow-up queue.

**Q: How are stock-negative and low material stock alerts handled?**
A: Stock-negative appears on relevant pages/summaries. Low material stock is not automatic first; use summaries and waiting-material notes.

**Q: What happens after delivery marks `ส่งออกแล้ว`?**
A: The Shipment enters admin shipment confirmation queue.

**Q: How are failed saves/actions shown?**
A: Inline error or toast.

**Q: Is there read/unread or dismiss notification state?**
A: No. Queue/work state is the source of truth, and alerts resolve by action/status change.

**Q: Are alert thresholds configurable now?**
A: Use defaults first. Detailed threshold settings are later.

**Q: Is notification history included?**
A: No separate history first. Use Activity Log / Management Log plus clear event structure.

**Q: What is the notification principle?**
A: Queue/status/event is the notification system.

### System Settings Decision Gates

**Q: What settings are needed first?**
A: Product settings, finance lists, Customer Tier, expense categories, and basic operational thresholds.

**Q: Where should settings live?**
A: Main `ตั้งค่า` plus contextual mini managers where useful.

**Q: Are Product Settings still as documented?**
A: Yes.

**Q: Is Customer Tier configurable?**
A: Yes.

**Q: Does Customer Tag need defaults?**
A: No.

**Q: Which finance/service lists are configurable?**
A: Expense Category, Payment Method, Carrier, and Service Case Type as simple lists. Payment Method defaults to `โอนเงิน`.

**Q: Is Service Case Status configurable?**
A: No. Use fixed starting statuses, not a workflow builder.

**Q: Are aging and low-stock thresholds configurable?**
A: Aging uses defaults first with later/basic manager setting possible. Low-stock threshold is later.

**Q: Where do material unit/category settings live?**
A: Mini managers inside the material area.

**Q: How is Supplier/Store handled?**
A: One shared master with simple flags/contexts.

**Q: Are print templates, evidence requirements, dashboard cards, and report presets configurable?**
A: No deep settings first. Use fixed templates, simple tracking-or-one-photo evidence rule, fixed dashboard cards, and fixed report presets.

**Q: How are setting changes logged and closed?**
A: Settings changes go to Management Log. Used values cannot be deleted and should be closed/inactivated. Unused values can be deleted. Closed values remain visible in history with a closed badge.

**Q: Is settings import/export included?**
A: No.

**Q: What is the settings principle?**
A: Controlled lists only where they reduce chaos.

## 26. Printing and Documents

**Q: What printable docs are first scope?**
A: Delivery Note, Shipping Sheet, Payment Voucher.

**Q: Does Delivery Note show prices?**
A: No.

**Q: Does Shipping Sheet show COD?**
A: Yes, when relevant to that Shipment.

**Q: Do printed documents use snapshots?**
...
**Q: When can PV be printed?**
A: After payment is confirmed.

**Q: What is the PV number format?**
...
**Q: Can printing be started from mobile?**
A: Yes. Mobile can initiate printing, but the printed document/template format is the same as desktop.

**Q: What is the print/export principle?**
A: Document snapshots and simple outputs, with no accounting or tax promise.
...
A: PV line-item detail, full accounting, tax, quotation, channel analytics, payroll, full material master beyond light material stock, supplier deep management, BOM/costing, QC, media library, customer merge, carrier API, wholesale pricing, and Data Migration / Starting Data import planning for this decision-gate round.
...
## 28. Edge Decision Gates After Q150
...
A: No. In the starting workflow, COD is allowed only on the final Shipment round that completes delivery for the Order.
...
A: Shipping Sheet COD is permission-aware. Delivery Team can see/print COD amount for Shipments they are responsible for; unrelated users without COD visibility cannot.
