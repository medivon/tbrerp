...
A: เมื่อผู้มีสิทธิ์ finance ปิดรายการพร้อมหลักฐานรับเงินที่บังคับใช้ หรือหมายเหตุที่อธิบายได้พอสำหรับ audit trail. ยังไม่ใช่ full accounting approval.
...
A: No. COD follow-up is created automatically when Shipment carries COD, but it cannot be closed before the related Shipment is closed.
...
A: Rak Samuk Worker can use `ขอเสนอราคา` for assigned work until the related PV round is closed.
...
A: Yes. They use `ขอเสนอราคา` until the related PV round is closed.
...
A: Yes, one PV can include multiple Rak Samuk works for one worker/payment round.
