...
A: `ออเดอร์กำลังดำเนินการ`, `งานกำลังผลิต`, `รอสร้างรอบจัดส่ง`, `ยืนยันการจัดส่ง`, `งานผลิตต้องติดตาม`, `ติดตาม COD / Payment`.
...
A: No. The Review screen itself is the final confirmation step. Warnings and override fields appear inline and block confirmation until resolved.
...
A: โดยหลักควรมีเงิน/มัดจำ แต่สามารถ override ได้โดยผู้มีสิทธิ์พร้อมเหตุผล.

**Q: What if an Order has both ready-stock and custom work?**  
A: เฉพาะออเดอร์ที่มีทั้งสินค้าพร้อมส่งและงานสั่งทำเท่านั้นที่มี `แผนจัดส่ง`. Default เป็น `ส่งพร้อมกัน`; admin may choose `จัดส่งแยกได้`. Later changes must go through the appropriate Order management or shipment-selection flow and obey any existing Job/Shipment state.

**Q: What are the main Order tabs?**  
A: `กำลังดำเนินการ`, `ออเดอร์ทั้งหมด`, `ร่างออเดอร์`, and `ปิดแล้ว / ยกเลิก`.

**Q: What appears in `กำลังดำเนินการ`?**  
A: Only real Orders that are not operationally complete. Drafts do not appear there. Rows with custom work show a simple `มีงานสั่งทำ` label.

**Q: What are the main Order statuses?**  
A: `กำลังดำเนินการ`, `กำลังผลิต`, `พร้อมสร้างรอบจัดส่ง`, `ส่งบางส่วน`, `รอยืนยันการจัดส่ง`, `จัดส่งครบแล้ว`, and `ยกเลิก`.
...
A: `ทั้งหมด`, `กำลังดำเนินการ`, `กำลังผลิต`, `พร้อมสร้างรอบจัดส่ง`, `ส่งบางส่วน`, `รอยืนยันการจัดส่ง`, `จัดส่งครบแล้ว`, และ `ยกเลิก`.
...
A: `แก้ข้อมูลผู้รับในออเดอร์นี้`, `แก้ไขรายการสินค้า`, `แก้ไขงานสั่งทำ`, `จัดการรอบจัดส่ง`, `เปิดติดตามการเงิน`, และ `ยกเลิกออเดอร์`. ไม่ใส่ `แก้ข้อมูลลูกค้าหลัก` ในเมนูนี้.

**Q: How should customer/recipient editing be separated?**  
A: `แก้ข้อมูลผู้รับในออเดอร์นี้` สำหรับข้อมูลที่ใช้กับออเดอร์นี้/รอบจัดส่งใหม่. Customer master แก้จากหน้า Customer Detail/Profile โดยเข้าได้จากปุ่ม `เปิดลูกค้า` ในส่วนลูกค้า.

**Q: If Order recipient detail changes after a Shipment round was created, what happens?**  
A: ไม่กระทบรอบจัดส่งที่สร้างแล้ว. ถ้าต้องแก้รอบจัดส่งที่สร้างแล้ว ให้ไปแก้ในหน้า `รอบจัดส่ง`. ข้อมูลผู้รับในออเดอร์นี้มีผลกับรอบจัดส่งใหม่เท่านั้น.
...
**Q: Where should `แก้ข้อมูลผู้รับในออเดอร์นี้` appear?**  
A: มีทั้งในเมนู `จัดการออเดอร์` และใน section ผู้รับ/สรุปออเดอร์.

**Q: Does editing Order recipient detail need Review Changes?**  
A: No. ใช้ modal/drawer แก้แล้วบันทึกพร้อม log เพราะไม่กระทบรายการ/Job/stock และไม่แก้รอบจัดส่งเดิมอัตโนมัติ.

**Q: Can Order recipient detail use the Customer address book?**  
A: ได้ เลือกจาก Address Entry เดิมของลูกค้าหรือกรอกใหม่ได้.
...
**Q: If the Order has multiple line-specific delivery addresses, what does `แก้ข้อมูลผู้รับในออเดอร์นี้` edit?**  
A: แก้เฉพาะผู้รับหลักของออเดอร์. รายการที่ส่งแยกต้องแก้ในรายการหรือรอบจัดส่งของตัวเอง.
...
A: ได้เฉพาะกรณี Job ยังไม่เริ่มผลิตจริง และต้องยกเลิก/ปิด Job พร้อมเหตุผลและ log.
...
A: ไม่มีหน้าแก้ไขออเดอร์ทั้งหมดหน้าเดียว. Order Detail เป็นหน้ารายงานหลัก และแก้เป็นส่วนๆ: ผู้รับผ่าน modal/drawer, รายการผ่าน `แก้ไขรายการออเดอร์`, รายละเอียดผลิตของ `JOB-O` ผ่าน Job, รอบจัดส่งผ่าน Shipment, และการเงินผ่าน Financial Follow-up.
...
A: งานทั่วไปทำได้ด้วยสิทธิ์แก้ Order ผ่าน Review Changes + เหตุผล. งานเสี่ยงสูง เช่น ยกเลิกออเดอร์, ลบงานสั่งทำที่มี `JOB-O`, หรือแก้ยอดหลังมีการรับเงิน ต้องใช้สิทธิ์สูงกว่า/Manager.
...
**Q: If `ส่งพร้อมกัน` blocks shipment but the admin needs to split shipment, where is it changed?**  
A: จาก section `จัดการรอบจัดส่ง` โดยแสดงปุ่ม `เปลี่ยนเป็นจัดส่งแยกได้` เมื่อสถานะยังอนุญาต พร้อม log.

**Q: Where is line-specific delivery detail edited?**  
A: จากแถวรายการนั้นโดยตรง เช่น `แก้ข้อมูลจัดส่งรายการนี้`. ถ้ารายการเข้า Shipment แล้ว ให้ไปแก้ใน Shipment.

**Q: If the main Order recipient changes before any Shipment exists, what happens to lines?**  
A: รายการที่ไม่ได้ตั้งค่าส่งแยกจะใช้ผู้รับหลักใหม่อัตโนมัติ. รายการที่มีข้อมูลจัดส่งเฉพาะจะไม่ถูกแก้.
...
A: บังคับเมื่อยอดรวมเปลี่ยน, ลบรายการ, หรือกระทบ Job/Shipment/Stock. การแก้ข้อความเล็กๆ ที่ไม่กระทบ downstream ไม่ต้องบังคับ.
...
A: Yes, by setting; warn but can proceed.
