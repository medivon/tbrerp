...
- Data Migration / Starting Data import planning in this decision-gate round
...
- Admin Dashboard does not show money amounts. It may show operational finance counts such as unresolved COD/Payment follow-up count, but not baht values.
- Admin Dashboard does not show Owner or Current Handler as primary card/preview signals.
- Admin Dashboard has no personal task mode first; shared queues come first.
- Admin Dashboard can show stock-negative warning summary, overdue delivery, waiting materials, and urgent/old work through critical preview or relevant queue signals.
- Low-stock and Service Case counts are not main dashboard signals in the starting screen.
- Individual performance reporting is deferred, but data should remain clean enough for a later User Dashboard design.

## Operational Alerts

Starting notification behavior:

- In-app alerts on the actual working screens only.
- Alert unit is queue item, status chip, badge/count, inline warning, toast/error, or critical preview.
- No separate read/unread notification inbox.
- No dismissible alerts; alerts resolve when the underlying action/status changes.
- No separate notification history screen first; use Activity Log / Management Log.
- Keep module/event names explicit so future API, webhook, or external hook integration can be added.

Important first alert events:

- Urgent Job
- Near delivery date
- Old/aged work
- Waiting materials
- Job Revision awaiting acknowledgement
- Rak Samuk missing price / approval
- COD or Payment follow-up
- Payment Audit follow-up
- Stock-negative warning
...
- Read/unread inbox
- Detailed threshold settings
- Low material stock as an automatic alert

## Management Dashboard and Reports

First report/dashboard principle:

- Operational clarity before analytics.
- Manager view emphasizes unfinished work, urgency, blockers, and age.
- Report screens start with summary cards and tables.

First management/sales/report rules:

- First management report: unfinished work by department/status.
...
Starting settings behavior:

- Use a main `ตั้งค่า` area plus mini managers in context.
- Product Settings remain as documented.
- Customer Tier is configurable.
- Customer Tag has no required default set.
- Expense Category and Payment Method are simple controlled lists.
- Payment Method defaults to `โอนเงิน`.
- Carrier is a simple list, with free text allowed in selected cases.
- Service Case Type is a simple controlled list.
- Service Case Status is fixed first, not a configurable workflow.
- Aging thresholds use defaults first; later/basic manager setting is possible.
- Low-stock threshold setting is later.
- Material unit and material category are mini managers in the material area.
- Supplier/Store is one shared master with simple flags/contexts.
...
- Product Purchase Order creates Payment Audit Follow-up only after full receipt.
- Material Purchase Order creates Payment Audit Follow-up when the whole document is received into material stock.
- Payment Audit Follow-up does not create Expense Entry automatically.
...
- Uses actual payment date for expense reports
- Can record Service Case refunds

Finance reports:

- Rough profit may be shown as sales minus expenses.
