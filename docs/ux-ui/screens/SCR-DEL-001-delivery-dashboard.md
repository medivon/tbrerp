...
- Shipment card: recipient, address summary, carrier, item thumbnail/summary, evidence status, action `เปิดรอบจัดส่ง`.
- Detail/evidence view: large delivery information, item list, tracking field, `รูปหลักฐานจัดส่ง` upload, note, `ส่งออกแล้ว`.
...
| Tracking | Tracking | TH123456789 | Shipment Evidence | Optional if a delivery evidence photo exists. |
| Evidence photo | รูปหลักฐานจัดส่ง | รูปพัสดุขึ้นรถ | Shipment Evidence | One multi-photo field. Required only when tracking is blank. |
...
| Add tracking/evidence | เพิ่มหลักฐานจัดส่ง | Delivery Team | Adds tracking or delivery evidence photos. | No |
...
| Evidence needed | ต้องมี Tracking หรือรูป | Neither tracking nor delivery evidence photo exists yet. | Warning chip near send-out action. |
...
- Required evidence missing: show `กรุณาเพิ่ม Tracking หรือรูปหลักฐานจัดส่ง` and keep `ส่งออกแล้ว` blocked.
...
- Delivery Team does not see COD amount in the system UI. COD follow-up belongs to admin/audit/finance; COD amount appears on the printed Shipping Sheet where relevant.
...
- Evidence capture should be obvious near the send-out action: tracking or one delivery evidence photo is enough.
...
- None for the delivery evidence rule. Visual structure still waits for mobile/tablet app-shell approval.
