...
- Check quantity, delivery date, urgency, and payment/override signals.
...
- Right or inline readiness checklist: required custom-work detail, image completeness, payment/override signal.
...
| Payment signal | การชำระเงิน | ยังไม่มีรายการรับเงิน | Payment Term / Payment Record | Permission-aware; override handled on Review. |
...
| Needs override | ต้องอนุมัติพิเศษ | Payment condition requires higher-permission reason. | Prominent permission chip on Review. |
...
- Payment override required: show a warning here and resolve the override inline on Order Review.
...
- Creating `JOB-O` without Payment Record requires permission override and reason on Order Review.
