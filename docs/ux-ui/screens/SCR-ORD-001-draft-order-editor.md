...
- `เพิ่มสินค้าพร้อมส่ง` opens Product Model-first selection: choose Product Model, then choose enabled color/SKU Variant.
...
- Product Model-first ready-stock selector
- Color/SKU Variant selector inside the selected Product Model
...
| Customer primary phone | เบอร์หลักลูกค้า | 081-234-5678 | Customer | Customer primary phone is required before real Order creation. |
| Customer Social | Social | Facebook: Malee Teak Home | Customer Social Contact | Optional CRM identity for future message connection. |
| Customer tier | ระดับลูกค้า | ลูกค้า VIP | Customer Tier | May provide default final discount for this new Order. |
| Tier discount | ส่วนลดตามระดับลูกค้า | สินค้าปกติ 5% / งานสั่งทำ 0% | Customer Tier Discount | Optional percentage discount split by line type; suggested before final Order total summary and editable by permission. Manual edits log old/default and new values; discount amount is rounded to whole baht in Order summary. |
| Recipient | ผู้รับสินค้า | คุณภพ | Address Entry | Recipient can differ from Customer. |
| Address | ที่อยู่จัดส่ง | 99/12 หมู่ 4 ต.บางรัก อ.เมือง จ.เชียงใหม่ 50000 | Address Entry / Order Recipient Detail | Use structured address fields; becomes the Order recipient snapshot after confirmation; save back to Customer only when explicitly checked. |
| Ready-stock line | สินค้าพร้อมส่ง | เก้าอี้ไม้สักสีธรรมชาติ | Order Line | Reserves stock only after Order confirmation. |
| Product Model | SKU หลัก | โต๊ะข้างไม้สัก | Product Model | Ready-stock selection shows Product Models first. |
| SKU Variant / color | สี / SKU ย่อย | สีโอ๊คเข้ม / TBR-TBL-123-OAK | SKU Variant | Selected after Product Model. |
| Sellable stock | ขายได้ | 2 ชิ้น / หมด | Computed stock | Display stock in Order selection with this term only. |
...
| Choose no-stock product | เลือกสินค้าที่ไม่มีสต๊อก | Admin / permitted sales | Allows selecting enabled SKU Variant colors that currently show `หมด`. | No |
...
| Sold out | หมด | Selected SKU Variant has `ขายได้` equal to 0. | Warning in selector and Review if selected. |
| Quantity exceeds sellable stock | จำนวนเกินที่ขายได้ | Selected quantity is higher than `ขายได้`. | Warning in selector and Review if selected. |
...
- Sold out / quantity exceeds `ขายได้`: show warning immediately in the line editor and again in Order Review.
- If the user searched by SKU Variant code, show the Product Model result and highlight the matching color/SKU Variant.
- Order Review rechecks latest stock before confirmation; stale stock warnings disappear automatically when stock is sufficient.
...
- Creating a new Customer from Order Create requires complete Order-required Customer data before saving the Customer: customer name, primary phone, house/address detail, subdistrict, district, province, and postal code.
- Deactivated Customers are not shown in the Order Create customer selector.
- If the selected Customer tier is `ระวังเป็นพิเศษ`, user must acknowledge the warning before continuing and the acknowledgement is logged.
...
- If a Customer already has 3 saved addresses, a new address can still be used as the Order Recipient Detail snapshot but cannot be saved back to Customer.
...
- Draft Order does not reserve stock; ready-stock reservation happens only when the real Order is confirmed.
...
- In `เพิ่มสินค้าพร้อมส่ง`, show Product Models first and let the user choose a color/SKU Variant inside the product.
- Default item selection shows only products with `ขายได้ > 0`; use `เลือกสินค้าที่ไม่มีสต๊อก` for enabled colors that are `หมด`.
- Search ready-stock selection by product name, product code, and SKU Variant code.
- Use `ขายได้ X ชิ้น` and `หมด`; do not mix in `คงเหลือ` or `พร้อมขาย` in the Order selection UI.
