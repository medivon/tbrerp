...
- Order Shipment Plan control only when mixed line types exist: default `ส่งพร้อมกัน`, optional `จัดส่งแยกได้`
- Price input
...
| Address | ที่อยู่จัดส่ง | 99/12 ถ.เจริญกรุง กรุงเทพฯ | Address Entry | Saved under Customer; later Shipment snapshots it. |
...
| Shipment plan | แผนจัดส่ง | ส่งพร้อมกัน | Order Shipment Plan | Required only when Order mixes ready-stock and custom work. |
...
| Set shipment plan | ส่งพร้อมกัน / จัดส่งแยกได้ | Admin / permitted sales | Controls later shipment creation for mixed Orders. | No |
...
- Payment override needed for custom work without payment: show warning; final override happens inline on Review.
...
- Creating `JOB-O` without Payment Record requires permission override and reason on Review.
