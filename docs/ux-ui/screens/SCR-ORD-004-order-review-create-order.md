...
| Customer Social | Social | Facebook: Malee Teak Home | Customer Social Contact | Optional CRM identity when present. |
| Customer tier | ระดับลูกค้า | ลูกค้า VIP | Customer Tier | Review customer level used for default discount context. |
| Tier discount | ส่วนลดตามระดับลูกค้า | สินค้าปกติ 5% / งานสั่งทำ 0% | Customer Tier Discount | Optional percentage discount split by line type; final value is rounded to whole baht, logged, and snapshotted. |
...
| Address | ที่อยู่จัดส่ง | 99/12 หมู่ 4 ต.บางรัก อ.เมือง จ.เชียงใหม่ 50000 | Address Entry / Order Recipient Detail | Structured address review; later Customer address changes do not rewrite this Order. |
...
| Special caution customer | ระวังเป็นพิเศษ | Customer tier warning acknowledged before confirmation. | Warning chip with acknowledgement log. |
...
- Customer tier caution not acknowledged: block confirmation until user acknowledges `ระวังเป็นพิเศษ`; no manager approval required.
