...
All Orders List shows every real Order across the main Order statuses. It is the archive/search-heavy Order tab used by admins when they need to find an Order, confirm customer details, check item summaries, see shipment state, and open Order Detail.
...
- Know whether shipment has not started, has a round but no tracking yet, is partially shipped, or has tracking.
...
- Order workspace tabs from `กำลังดำเนินการ`, `ร่างออเดอร์`, or `ปิดแล้ว / ยกเลิก`.
