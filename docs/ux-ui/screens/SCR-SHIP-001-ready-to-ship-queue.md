...
The Ready-to-Ship Queue is the admin `รอสร้างรอบจัดส่ง` screen. It shows Orders with items that are ready for admin to create a Shipment under the Order Shipment Plan: ready-stock Order Lines, completed `JOB-O` work, and ready Service Case items.
...
- Respect `ส่งพร้อมกัน` vs `จัดส่งแยกได้` so ready-stock items are not shipped early by mistake.
...
- Mixed ready-stock/custom Orders appear only when the Order Shipment Plan allows the currently ready items to ship.
...
| Shipment plan | แผนจัดส่ง | ส่งพร้อมกัน / จัดส่งแยกได้ | Order Shipment Plan | Controls whether partial ready items can be shipped. |
...
| Bulk eligible | สร้างรวมได้ | Order can be included in bulk shipment creation under confirmed rules. | Subtle blue/green chip. |
...
| Ship together | ส่งพร้อมกัน | Mixed Order must wait until all required lines are ready before shipping. | Show only as explanatory context when relevant. |
| Separate allowed | จัดส่งแยกได้ | Ready-stock or completed custom lines may ship separately. | Planning chip. |
| Draft preparing | ร่างรอบจัดส่ง | Item is already in Draft Shipment. | Prevent duplicate Shipment; not the default focus of this screen. |
...
- Item already moved into Draft Shipment by another admin: show `รายการนี้กำลังถูกเตรียมรอบจัดส่งแล้ว`.
...
- For mixed Orders, ready-stock lines appear here before custom completion only when the Order Shipment Plan is `จัดส่งแยกได้`.
- If the Order Shipment Plan is `ส่งพร้อมกัน`, the Order should wait until all required lines are ready.
...
- Use bulk actions only for Orders that are safe under confirmed business rules; other Orders should require opening the row or Shipment Builder.
- Respect the Order Shipment Plan before enabling row or bulk shipment creation.
